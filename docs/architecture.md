...
- Worker entry point with live `/api/public/summary`, `/api/public/services`, and `/api/public/incidents` routes
...
- maintenance API backed by config or D1
...
2. The Worker returns summary, service, or incident payloads from D1-backed state.
